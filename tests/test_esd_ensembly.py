...
from my_code_base.esd.ensemble import *
...
def dummy_data():
    ...
    member_ids = ['r{}i{}p1f1'.format(r, np.random.randint(1, 3)) for r in range(1,11)]*5
    combined_ids = [f"{model_id}.{member_id}" for model_id, member_id in zip(model_ids, member_ids)]
...
def test__build_member_mapping_table():
    member_values = ['ACCESS-CM2.r1i1p1f1', 'ACCESS-CM2.r2i1p1f1', 'ACCESS-CM2.r3i1p1f1']
