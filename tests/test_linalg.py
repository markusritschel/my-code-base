...
def test_inv_ndarray():
    ...
    assert np.allclose(result, expected), (
        "The function does not return the expected result for ndarray input"
    )
...
def test_empirical_covariance_dataframe():
    ...
    expected = pd.DataFrame(
        np.array([[72, 180, 180], [180, 450, 450], [180, 450, 450]]),
        index=[1, 2, 3],
        columns=[1, 2, 3],
    )
...
def test_empirical_covariance_ndarray():
    ...
    expected = np.array([[72, 180, 180], [180, 450, 450], [180, 450, 450]])
    assert np.all(result == expected), (
        "The function does not return the expected result for ndarray input"
    )
