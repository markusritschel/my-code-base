...
def weighted_annual_mean(ds: xr.Dataset | xr.DataArray):
    ...
    assert np.allclose(weights.groupby("time.year").sum(xr.ALL_DIMS), 1.0), "The sum of the weights should be 1.0!"

    # Setup our masking for nan values
    ones = xr.where(ds.isnull(), 0.0, 1.0)

    # Calculate the annual values, weighted by days in each month
    ds_sum = (ds * weights).resample(time="AS").sum(dim="time", keep_attrs=True)
    ...
    ones_out = (ones * weights).resample(time="AS").sum(dim="time", keep_attrs=True)
...
def xr_deseasonalize(ds, freq=12, dim='time'):
    ...
    detrended = ds - trend
    detrended_deseasonalized = detrended.groupby(f"{dim}.month") - detrended.groupby(f'{dim}.month').mean()
